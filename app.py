...
def index():
    ...
    top_category = playstore.groupby('Category').count()[['App']].sort_values(by='App', ascending=False).reset_index()
    top_category.rename({'App': 'Jumlah'}, axis=1, inplace=True)
    ...
    game = df2[df2.Category == 'GAME']
    game = game.Genres.value_counts().head(10).sort_values()
    fig = plt.figure(figsize=(5, 5))
    fig.add_subplot()
    plt.barh(game.index, game.values, color='darkblue')
    plt.xlabel('Genre')
    plt.ylabel('Frequency')
    plt.savefig('static/game_genre.png', bbox_inches="tight")
